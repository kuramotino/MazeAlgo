...
namespace Algorizm
{
	...
	void IssueCommand::Tansaku()
	{
		...
		if (my_micemgr->RetKasokuEnd()==true && !isStop && isStart && !my_micemgr->RetStartPause())
		{
			...
			nextVec = (!my_plan->RetReturn()) ? my_plan->Adati(4, goal_pos) : my_plan->search_unknown_dijkstra(4, goal_pos);
			switch (nextVec)
			{
			case Left:
			...
			case Back:
				...
				my_plan->search_unknown_set_goal_pos(4, goal_pos);//back中にダイクストラ法で最短経路上の未知区間を探す
				break;
			}
